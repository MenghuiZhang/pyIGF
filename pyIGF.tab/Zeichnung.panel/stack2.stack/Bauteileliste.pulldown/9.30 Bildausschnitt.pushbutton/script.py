...
def BAermitteln(ids,Cate,Ansich,dict):
    title = "{value}/{max_value} Bauteile in " + Cate+ ' in Ansicht ' + Ansich
...
for ele in View_dict.Keys:
    ...
    RZ = DB.FilteredElementCollector(doc,view).OfCategory(DB.BuiltInCategory.OST_PipeAccessory).WhereElementIsNotElementType().ToElementIds()
    LZ = DB.FilteredElementCollector(doc,view).OfCategory(DB.BuiltInCategory.OST_DuctAccessory).WhereElementIsNotElementType().ToElementIds()
    LD = DB.FilteredElementCollector(doc,view).OfCategory(DB.BuiltInCategory.OST_DuctTerminal).WhereElementIsNotElementType().ToElementIds()
    LKF = DB.FilteredElementCollector(doc,view).OfCategory(DB.BuiltInCategory.OST_DuctFitting).WhereElementIsNotElementType().ToElementIds()
    LK = DB.FilteredElementCollector(doc,view).OfCategory(DB.BuiltInCategory.OST_DuctCurves).WhereElementIsNotElementType().ToElementIds()
    ...
    BAermitteln(LK,'Luftkanäle',ele,element_dict)
    BAermitteln(LKF,'Luftkanalformteile',ele,element_dict)
...
if forms.alert("Daten schreiben?", ok=False, yes=True, no=True):
    ...
    step = min(int(len(elems)/100)+1,500)
    with forms.ProgressBar(title=title, cancellable=True, step=step) as pb:
        n = 0
